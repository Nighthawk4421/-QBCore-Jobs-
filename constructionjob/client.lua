...
RegisterNetEvent('constructionjob:client:renttruck', function()
    local vehicle = 'f35019'
    local model = 'f35019'
    local vehicle2 = 'ct660dump'
    local model2 = 'ct660dump'
    local player = PlayerPedId()
    local hasItem = QBCore.Functions.HasItem('jobdetails')
    if hasItem then
        TriggerEvent('animations:client:EmoteCommandStart', {"tablet2"})
        QBCore.Functions.Progressbar('truckerjob_delivery', 'Renting vehicles for the day ', 10000, false, true, {
            disableMovement = true,
            disableCarMovement = true,
            disableMouse = false,
            disableCombat = true,
        ...
        QBCore.Functions.SpawnVehicle(model, function(vehicle)
            SetEntityHeading(vehicle, 297.68)
        ...
        QBCore.Functions.SpawnVehicle(model2, function(vehicle2)
            SetEntityHeading(vehicle2, 316.42)
            --TaskWarpPedIntoVehicle(player, vehicle, -1)
            TriggerEvent("vehiclekeys:client:SetOwner", GetVehicleNumberPlateText(vehicle2))
...
RegisterNetEvent('constructionjob:client:rentpaletotruck', function()
    local vehicle = 'duramax'
    local model = 'duramax'
    local player = PlayerPedId()

    TriggerEvent('animations:client:EmoteCommandStart', {"tablet2"})
    QBCore.Functions.Progressbar('truckerjob_delivery', 'Renting vehicle for delivery ', 10000, false, true, {
        disableMovement = true,
        disableCarMovement = true,
        disableMouse = false,
        disableCombat = true,
    ...
    QBCore.Functions.SpawnVehicle(model, function(vehicle)
        SetEntityHeading(vehicle, 143.99)
        TaskWarpPedIntoVehicle(player, vehicle, -1)
        TriggerEvent("vehiclekeys:client:SetOwner", GetVehicleNumberPlateText(vehicle))
        SetVehicleEngineOn(vehicle, true, true)
        SetVehicleExtra(vehicle, 2, 1)
